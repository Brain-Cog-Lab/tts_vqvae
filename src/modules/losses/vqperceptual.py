...
def adopt_weight_epoch(weight, epoch, threshold=0, value=0.):
    if epoch < threshold:
        weight = value
    return weight
...
class VQLPIPSWithDiscriminatorEpoch(nn.Module):
    """
    ...
    def __init__(self,
        ...
        self.discriminator = NLayerDiscriminator(
            ...
            use_actnorm=use_actnorm,
            ndf=disc_ndf).apply(weights_init)
        self.discriminator_iter_start = disc_start_epoch
        if disc_loss == "hinge":
            self.disc_loss = hinge_d_loss
        elif disc_loss == "vanilla":
            self.disc_loss = vanilla_d_loss
            ...
            raise ValueError(f"Unknown GAN loss '{disc_loss}'.")
        print(f"VQLPIPSWithDiscriminatorEpoch running with {disc_loss} loss.")
        self.disc_factor = disc_factor
        self.discriminator_weight = disc_weight
        self.disc_conditional = disc_conditional
    ...
    def forward(self,
        ...
        if optimizer_idx == 0:
            ...
            disc_factor = adopt_weight_epoch(
                self.disc_factor,
                epoch,
                threshold=self.discriminator_iter_start)
            loss = nll_loss + d_weight * disc_factor * g_loss + self.codebook_weight * codebook_loss.mean(
